...
if (isLoggedOn()) {
    ...
    if (isset($_POST["lstidT"])){
        ...
        for ($i = 0; $i < count($lstidT); $i++){
            \modele\dao\TypeDAO::delete($idU, $lstidT[$i]);
        }
    }

    if(isset($_POST["lstidTadd"])){
        $lstidTadd = $_POST["lstidTadd"];
        for($i = 0; $i < count($lstidTadd); $i++) {
            var_dump($lstidTadd);
            \modele\dao\TypeDAO::insert($idU, $lstidTadd[$i]);
        }
    }
...
// Si on a changé le mot de passe, il faut se reconnecter
    if (!isLoggedOn()) {
        ...
        $mesTypesAimes = \modele\dao\TypeDAO::getTypesByUtilsateur($idU);

        $mesTypesnonAimes = \modele\dao\TypeDAO::getAllNonPreferesByIdU($idU);
