...
class RestoDAO {
    ...
    public static function getTop4(): array {
        $lesObjets = array();
        try {            
            $requete = "SELECT SUM(note) AS NotesCumulees, r.idR, note, nomR, numAdrR, voieAdrR, cpR, villeR, latitudeDegR, longitudeDegR, descR, horairesR  
                       FROM resto r
    ...
    public static function getRestosByTypes(string $lesTypes): array{
        $lesRestos = array();
        try {
            
            $requete = "SELECT * FROM resto INNER JOIN typeresto ON resto.idR = typeresto.idR INNER JOIN type ON typeresto.idTC = type.idTC WHERE type.libelleTC IN ('".$lesTypes."') GROUP BY nomR;";
            $stmt = Bdd::getConnexion()->prepare($requete);
            $ok = $stmt->execute();
            ...
            if ($ok) {
                ...
                while ($enreg = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    //Instancier un nouveau restaurant et l'ajouter à la liste
                    $lesRestos[] = self::enregistrementVersObjet($enreg);
                }
            }
        } catch (PDOException $e) {
            throw new Exception("Erreur dans la méthode " . get_called_class() . "::getAimesByIdU : <br/>" . $e->getMessage());
